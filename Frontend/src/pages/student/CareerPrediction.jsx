...
import CareerProgressSection from "../../components/CareerProgressSection";
...
const TOOLTIP_DATA = {
  aptitude: {
    ...
    meaning:
      "A measure of your logical reasoning, problem-solving, and mathematical ability.",
    purpose:
      "Used to gauge your suitability for roles requiring heavy analytical thinking like Data Science or Engineering.",
    example:
      "If you consistently score high in math and logic puzzles, set this to 80% or above.",
  },
  communication: {
    title: "Communication Skills",
    meaning: "Your ability to convey ideas clearly, both verbally and in writing.",
    purpose:
      "Critical for Management, Consulting, and Team Lead roles where coordination is key.",
    example:
      "If you are comfortable presenting to groups and writing clear documentation, set this to 85%.",
  },
  risk: {
    title: "Risk Tolerance",
    meaning:
      "How comfortable you are with job uncertainty in exchange for potential high rewards.",
    purpose:
      "Helps decide if you are better suited for a stable corporate job or a fast-paced Startup.",
    example:
      "A score of 90% means you are happy to join a new startup with high equity potential despite lower job security.",
  },
};
...
const APTITUDE_QUESTIONS = [
  {
    id: 1,
    q: "If a project takes 10 days with 2 developers, how many days for 5 developers?",
    options: ["2 days", "4 days", "5 days", "25 days"],
    correct: "4 days",
  },
  {
    id: 2,
  ...
  {
    id: 3,
    q: "A store offers 20% off on a ₹1000 item. What is the final price?",
    options: ["800", "850", "900", "200"],
    correct: "800",
  },
  { id: 4, q: "Solve: (25 * 4) / 10 + 5", options: ["10", "15", "20", "25"], correct: "15" },
  {
    id: 5,
    q: "Find the odd one out: 27, 64, 81, 125",
...
export default function CareerPrediction() {
  ...
  const branchOptions =
    BRANCH_CONFIG[form.branch] || { programming: [], technical: [], tools: [], interests: [] };

  // --- C. Logic Handlers ---

  const calculateProficiency = () => {
    const score =
      parseInt(testForm.projects) * 2 +
      parseInt(testForm.internships) * 5 +
      parseInt(testForm.certifications) * 3 +
      parseInt(testForm.yearsOfExperience) * 4;

    const result = score >= 25 ? "Advanced" : score >= 10 ? "Intermediate" : "Beginner";
 ...
 const predictCareer = async () => {
  ...
  try {
    ...
    // Save to your Supabase so it shows on Dashboard (RLS: you can insert your own row)
    if (recs.length > 0 && user?.id) {
      const { error: saveErr } = await supabase.from("career_predictions").insert({
        user_id: user.id,
        ...
        tools_frameworks: payload.Tools_Frameworks,
        skill_proficiency: payload.Skill_Proficiency,
        career_preference: payload.Career_Preference,
        research_or_industry: payload.Research_Or_Industry,
        aptitude_score: payload.Aptitude_Score,
        communication_skill: payload.Communication_Skill,
        risk_tolerance: payload.Risk_Tolerance,
        top_career: recs[0]?.career ?? null,
        top_match_score: recs[0]?.matching_score ?? null,
        recommendations: recs,
      });
      if (saveErr) {
        console.error("Career save failed:", saveErr);
        setStatus({ type: "success", message: "Prediction done, but save failed. Check console." });
      } else {
        setStatus({ type: "success", message: "Saved! Go to Dashboard to see your career path." });
      }
    } else {
    ...
    setStatus({
      type: "error",
      message: "Backend server gone on a lunch break.",
    });
  } finally {
    setLoading(false);
  }
  ...
  const InfoPopup = ({ id }) => {
    ...
    return (
      <div className="absolute z-50 bottom-full mb-3 w-72 p-4 bg-slate-900 text-white text-[13px] rounded-xl shadow-2xl animate-in fade-in zoom-in duration-200">
        <h4 className="font-bold text-indigo-400 mb-1 uppercase tracking-tight">
          {info.title}
        </h4>
        <p className="mb-2 opacity-90">{info.meaning}</p>
        <div className="pt-2 border-t border-slate-700/50 italic text-indigo-200">
  ...
  // ✅ NEW INPUT BASED SKILL CATEGORY (Typing + Chips)
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 h-11 px-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500"
          />

          <button
            type="button"
            onClick={addItem}
            className="px-4 h-11 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold shadow-md transition-all active:scale-95"
          >
            Add
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {(selected || []).length === 0 ? (
            ...
            selected.map((item) => (
              <div
                key={item}
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-semibold bg-indigo-600 border-indigo-600 text-white shadow-md"
              >
                {item}
                <button
                  type="button"
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {options.slice(0, 7).map((opt) => (
              <button
                ...
                className="px-3 py-1.5 rounded-lg border text-xs font-semibold bg-white dark:bg-slate-700 border-slate-200 text-slate-600 dark:text-slate-200 hover:bg-indigo-50"
              >
              {opt}
              </button>
            ))}
      ...
      <div className="p-6 max-w-6xl mx-auto min-h-screen transition-colors duration-300">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="flex flex-col gap-2">
              ...
              <select
                ...
                className="w-full h-12 px-4 bg-slate-50 dark:bg-slate-900 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500"
              >
                {Object.keys(BRANCH_CONFIG).map((b) => (
        ...
        <section className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 mb-10 overflow-visible transition-colors">
          ...
          <div className="p-8 space-y-10">
            <div className="grid md:grid-cols-3 gap-10">
              <SkillCategory
                title="Programming"
                options={branchOptions.programming}
                selected={form.programmingSkills}
                field="programmingSkills"
              />

              <SkillCategory
                title="Technical"
                options={branchOptions.technical}
            ...
            <div className="pt-8 border-t border-slate-100 dark:border-slate-700 flex flex-col md:flex-row md:items-center gap-6 relative z-20">
              ...
              <div className="flex-1 flex flex-col sm:flex-row gap-4 items-center">
                <button
                  ...
                  className="w-full sm:w-auto py-3 px-8 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg transition-all active:scale-95"
                >
        ...
        <section className="bg-white dark:bg-slate-800 p-8 rounded-2xl mb-10 shadow-sm border border-slate-200 dark:border-slate-700 transition-colors">
          ...
          <div className="grid md:grid-cols-2 gap-8 items-start">
            {/* ✅ UPDATED: Area of Interest now typing + chips */}
            <SkillCategory
            ...
            {["aptitude", "communication", "risk"].map((scoreKey) => (
              <div key={scoreKey} className="flex flex-col gap-4 relative">
                <div className="flex justify-between items-center group relative">
                  <label
                    className="text-xs font-bold uppercase text-slate-500 cursor-help"
                    onMouseEnter={() => setActiveTooltip(scoreKey)}
                ...
                {scoreKey === "aptitude" ? (
                  <button
                    ...
                    className="w-full py-2.5 bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold rounded-xl shadow-md transition-all active:scale-95"
                  >
                    Take Aptitude Test
                  </button>
                ) : (
                  <input
                    ...
                    value={form[scoreKey]}
                    onChange={(e) =>
                      setForm({ ...form, [scoreKey]: +e.target.value })
                    }
        ...
        {status.message && (
          <div
            className={`mb-6 p-4 rounded-2xl text-center text-sm font-semibold animate-in fade-in duration-300 ${
              status.type === "error"
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-4 bg-indigo-600 text-white text-xl font-bold rounded-2xl shadow-xl hover:bg-indigo-700 active:scale-95 transition-all disabled:opacity-50"
        >
          ...
          <div className="mt-8 relative overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-xl">
            ...
            <div className="relative p-6">
              <div className="grid gap-4">
                ...
                score >= 75 ? "Best Career Fit" :
                score >= 55 ? "Strong Career Option" :
                score >= 35 ? "Potential Career Path" :
                      ...
                      <div className="relative p-5 flex flex-col gap-4">
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex items-start gap-3">
                            <div
                              className={`min-w-[44px] h-11 flex items-center justify-center rounded-2xl font-extrabold text-sm ${
                                isTop
                            ...
                            <div className="flex flex-col">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                <span
                                  className={`text-[10px] font-bold px-2 py-1 rounded-full border ${confidenceClass}`}
                                >
                                  {confidenceLabel}
                                </span>
                          ...
                          <div className="h-2.5 w-full rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden border border-slate-200 dark:border-slate-700">
                            <div
                              className={`h-full rounded-full transition-all duration-700 ${
                                isTop ? "bg-indigo-600" : "bg-indigo-500"
                              }`}
        ...
        {/* Career progress: YouTube, courses, actions – visible to teachers & admin */}
        <CareerProgressSection topCareerName={recommendations[0]?.career} />
        ...
        {showProficiencyTest && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl p-8 animate-in zoom-in duration-200 border border-transparent dark:border-slate-800">
              ...
              <button
                onClick={calculateProficiency}
                className="w-full py-4 bg-indigo-600 text-white font-bold rounded-2xl hover:bg-indigo-700 transition-all active:scale-95 shadow-lg shadow-indigo-200 dark:shadow-none"
              >
                Submit Assessment
              </button>

              <button
                onClick={() => setShowProficiencyTest(false)}
                className="w-full mt-2 py-2 text-slate-400 dark:text-slate-500 text-sm font-medium hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
              >
                Cancel
              </button>
            </div>
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
            <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl shadow-2xl animate-in zoom-in duration-200 overflow-hidden border border-transparent dark:border-slate-800">
              <div className="p-6 bg-orange-50 dark:bg-orange-900/20 border-b border-orange-100 dark:border-orange-900/30 flex justify-between items-center">
                ...
                <button
                  onClick={() => setShowAptitudeTest(false)}
                  className="text-orange-900/50 dark:text-orange-400/50 hover:text-orange-900 dark:hover:text-orange-400"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="p-8 space-y-6 max-h-[60vh] overflow-y-auto">
                {APTITUDE_QUESTIONS.map((q) => (
                  <div key={q.id}>
                    <p className="text-sm font-bold mb-3 text-slate-800 dark:text-slate-200">
                      {q.id}. {q.q}
                    </p>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {q.options.map((o) => (
                        <button
                          key={o}
                          onClick={() =>
                            setAptitudeAnswers({ ...aptitudeAnswers, [q.id]: o })
                          }
                          className={`py-2 text-xs border rounded-lg transition-all ${
                            aptitudeAnswers[q.id] === o
