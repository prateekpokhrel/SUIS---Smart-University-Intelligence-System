...
# 🔥 UPDATED: Import SEGA server
from SEGA.server import app as emergency_app   # SUIS SEGA
from auth_api import app as auth_app            # SUIS Auth & User API
...
if __name__ == "__main__":
    ...
    career_process = Process(target=run_career_api, daemon=True)
    spam_process = Process(target=run_spam_api, daemon=True)
    workload_process = Process(target=run_workload_api, daemon=True)
    emergency_process = Process(target=run_emergency_api, daemon=True)
    auth_process = Process(target=run_auth_api, daemon=True)
